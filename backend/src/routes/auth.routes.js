...
router.post('/connexion', [
  ...
  try {
    ...
    await prisma.utilisateur.update({ where: { id: utilisateur.id }, data: { derniereActivite: new Date() } })
    const { motDePasse: _, ...utilisateurSansMdp } = utilisateur
    res.json({ token: genererToken(utilisateur), utilisateur: utilisateurSansMdp })
  } catch (err) {
